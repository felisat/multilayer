...
def load_results(path, name, verbose=True):
    results_dict = np.load(os.path.join(path,name), allow_pickle=True)

    if verbose:
        print("Loaded results from "+os.path.join(path,name))
    return results_dict
...
class Experiment():
    ...
    def to_dict(self):
        ...
        return {'hyperparameters' : self.hyperparameters, 'hyperparameters_' : self.hyperparameters_, **self.results}
    ...
    def from_dict(self, hp_dict):
        ...
        del self.results["hyperparameters"]
